export default () => {
  ...
  const btnNova = container.querySelector("#novaDenuncia");
  ...
  async function carregarDenuncias() {
    try {
      ...
      const botoesExcluir = lista.querySelectorAll(".btnExcluir");
      ...
      botoesExcluir.forEach((btn) => {
        btn.addEventListener("click", async () => {
          ...
          await fetch(`http://localhost:5000/api/denuncias/${id}`, {
            method: "DELETE",
            headers: {
              Authorization: `Bearer ${token}`,
            },
